-- Enhanced Document Schema for Taxu (Safe Version)
...
DROP POLICY IF EXISTS "Users can update own tax forms" ON tax_forms;
CREATE POLICY "Users can update own tax forms" ON tax_forms
    FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own tax forms" ON tax_forms;
