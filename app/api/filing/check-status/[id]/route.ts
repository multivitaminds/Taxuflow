...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    console.log("[v0] Checking filing status for ID:", params.id)
    ...
    if (!filing) {
      console.log("[v0] Filing not found for ID:", params.id)
      return NextResponse.json({ error: "Filing not found", id: params.id }, { status: 404 })
    }
